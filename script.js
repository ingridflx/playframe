document.addEventListener('DOMContentLoaded', () => {
    ...
    // Novo container agrupado para os campos de registro
    ...
    async function fetchItems() {
        ...
        if (!token) return;
        try { const response = await fetch('http://localhost:3000/items', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    if(updateDuoBtn) {
        updateDuoBtn.addEventListener('click', async () => {
            ...
            const token = localStorage.getItem('playframe_token');
            const res = await fetch('http://localhost:3000/user/duocode', { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ duo_code: newCode }) });
    ...
    // Lógica para GERAR um código aleatório (COM TRAVA DE SEGURANÇA)
    ...
    if(generateDuoBtn) {
        generateDuoBtn.addEventListener('click', async () => {
            ...
            const token = localStorage.getItem('playframe_token');
            const res = await fetch('http://localhost:3000/user/duocode', { 
                method: 'PUT', 
    ...
    registerBtn.addEventListener('click', async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:3000/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            const data = await response.json();
            if (response.ok) {
    ...
    // --- ESQUECEU A SENHA (FLUXO SEGURO DE 2 PASSOS) ---
    ...
    // Envia o e-mail para receber o código (PASSO 1)
    forgotEmailForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Preenche o E-mail de forma visual no Passo 2
    ...
    // Valida o código e troca a senha (PASSO 2)
    ...
    itemForm.addEventListener('submit', async (e) => {
        ...
        try {
            if (editingId) { await fetch(`http://localhost:3000/items/${editingId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(payload) }); delete submitBtn.dataset.editingId; } 
            else { await fetch('http://localhost:3000/items', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(payload) }); }
    ...
    itemsContainer.addEventListener('click', async (e) => {
        ...
        if (e.target.classList.contains('delete-btn')) {
            if (confirm("Tem certeza que deseja excluir este item?")) { 
                ...
                // Aplica a classe CSS de encolhimento/fade-out
                itemCard.classList.add('removing');
                
                // Aguarda 300ms (tempo da animação) para fazer o delete no banco
                setTimeout(async () => {
            ...
            if (statusBtn) {
                ...
                if (itemToOperate.status !== newStatus) { await fetch(`http://localhost:3000/items/${itemId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ ...itemToOperate, status: newStatus }) }); fetchItems(); }
            }
        }
    });
